# tento soubor jsou je shrnutim ctyr puvodnich zdrojovych souboru Pisnovace v0.2
...
with open(SOURCE_DIR + SETTINGS_FILE_NAME) as file:
...
SLS_FONT_COLOR = lines[5].rstrip()
SLS_SHADOW_COLOR = lines[6].rstrip().split(",")[0]
SLS_SHADOW_SIZE = lines[6].rstrip().split(",")[1]
...
STG_STYLE_DICT = {"Tučné":"bold", "Kurzíva":"italic", "Žádné":"none"}

sizes_list = [str(i * 2) for i in range(10,50)]
text_style_list =["Tučné", "Kurzíva", "Žádné"]
shadow_size_list = [str(i) for i in range(-10,10)]
    ...
    for letter in old_text:
        if not in_bracket:
            ...
            if letter in [" ", "/", "|", "]"]:
                ...
                chars_before_chord = ""
                continue
            # povoleny znak pred akordem je zavorka
            if letter == "(":
                chars_before_chord += letter
            else:
                old_chord += letter
...
def sls_pure_image(color, event=None):
    ...
    if color == "hide":
        if sls_slides_canvas is not None and sls_presentation_window.winfo_width() == sls_target_slide_width:
            sls_slides_canvas.create_image(0, 0, image = background_image, anchor = "nw", tags = ("overlay", ))
        sls_preview_canvas.create_image(0, 0, image = preview_image, anchor = "nw", tags = ("overlay", ))
    else:
        if sls_slides_canvas is not None and sls_presentation_window.winfo_width() == sls_target_slide_width:
...
def sls_check_fullscreen(target_width_list):
    ...
    if size_x in target_width_list:
        ...
        image = Image.open(os.path.join(SOURCE_DIR, BG_SLIDESHOW_IMAGE_NAME))
        background_image = image.resize((size_x, size_y),Image.LANCZOS)
        background_image = ImageTk.PhotoImage(background_image)
        sls_slides_canvas.image = background_image
        sls_slides_canvas.create_image(0, 0, image=background_image, anchor="nw")
...
def sls_update_slide(text: str):
    ...
    if sls_slides_canvas is not None and sls_target_slide_width == sls_presentation_window.winfo_width():
        ...
        presentation_window_width = sls_presentation_window.winfo_width()
        presentation_window_height = sls_presentation_window.winfo_height()

        # stin pisma, pokud je zapnuty
        if SLS_SHADOW_SIZE != 0:
            #sls_slides_canvas.create_text(int(presentation_window_width / 2 + int(SLS_SHADOW_SIZE)),int(presentation_window_height / 2 + int(SLS_SHADOW_SIZE)),text=processed_text,fill=SLS_SHADOW_COLOR,font=SLS_FONT_STYLE,anchor=CENTER,justify=CENTER,width=sls_target_slide_width)

            sls_slides_canvas.create_text(
                int(presentation_window_width / 2 - 4),
                int(presentation_window_height / 2),
                text=processed_text,
                fill=SLS_SHADOW_COLOR,
                font=SLS_FONT_STYLE,
                anchor=CENTER,
                justify=CENTER,
                width=sls_target_slide_width,
            )

            sls_slides_canvas.create_text(
                int(presentation_window_width / 2),
                int(presentation_window_height / 2 - 4),
                text=processed_text,
                fill=SLS_SHADOW_COLOR,
                font=SLS_FONT_STYLE,
                anchor=CENTER,
        ...
        #sls_slides_canvas.itemconfig(textitem, width = presentation_window_width)

    preview_width = sls_preview_canvas.winfo_width()
    preview_height = sls_preview_canvas.winfo_height()
    sls_preview_canvas.delete("all")
    sls_preview_canvas.create_image(0,0,image = preview_image, anchor = "nw")
    font_size_scale = preview_width / sls_target_slide_width
    preview_font = (SLS_FONT_STYLE[0], int(int(SLS_FONT_STYLE[1]) * font_size_scale), SLS_FONT_STYLE[2] if len(SLS_FONT_STYLE) > 2 else "")
    sls_preview_canvas.create_text(int(preview_width/2)  + int(SLS_SHADOW_SIZE), int(preview_height/2)  + int(SLS_SHADOW_SIZE), fill = SLS_SHADOW_COLOR, font = preview_font, anchor = CENTER, text = processed_text, justify=CENTER, width=preview_width)
    sls_preview_canvas.create_text(int(preview_width/2), int(preview_height/2), fill = SLS_FONT_COLOR, font = preview_font, anchor = CENTER, text = processed_text, justify=CENTER, width=preview_width)
...
def stg_update_font_style(event = None):
    ...
    global SLS_FONT_COLOR, SLS_FONT_STYLE, SLS_SHADOW_COLOR, SLS_SHADOW_SIZE

    # zapise hodnoty do nastaveni
    with open(os.path.join(SOURCE_DIR,SETTINGS_FILE_NAME), "r") as file:
        lines_list = file.readlines()
    # ctvrty radek = tuple se stylem
    lines_list[4] = f"{stg_font_combobox.get()},{stg_font_size_box.get()}" + (("," + STG_STYLE_DICT[stg_font_style_box.get()]) if STG_STYLE_DICT[stg_font_style_box.get()] != "none" else "") + "\n"
    lines_list[5] = STG_COLOR_DICT[stg_font_color_box.get()] + "\n"
    lines_list[6] = STG_COLOR_DICT[stg_shadow_color_box.get()] + "," + stg_shadow_size_box.get() + "\n"
...
def stg_update_preview_canvas():
    ...
    stg_preview_image = image.resize((width + 1, height),Image.LANCZOS) # +1 kvuli float nepresnosti
    stg_preview_image = ImageTk.PhotoImage(stg_preview_image)
    stg_preview_canvas.delete("all")
    stg_preview_canvas.create_image(0,0,image = stg_preview_image, anchor = "nw")
    stg_preview_canvas.update()
...
def stg_update_preview_text():
    ...
    font_size_scale = width / main_window.winfo_width()
    preview_font = (SLS_FONT_STYLE[0], int(int(SLS_FONT_STYLE[1]) * font_size_scale), SLS_FONT_STYLE[2] if len(SLS_FONT_STYLE) > 2 else "")

    # pismo i se stinem
    stg_preview_canvas.delete("stg_text")
 ...
 kdo se na kříž podívá - bude žít, bude žít. """
    ...
    stg_preview_canvas.create_text(int(width/2)  + int(SLS_SHADOW_SIZE), int(height/2)  + int(SLS_SHADOW_SIZE), fill = SLS_SHADOW_COLOR, font = preview_font, anchor = CENTER, text = text, justify=CENTER, tags = ("stg_text",), width=width)
    stg_preview_canvas.create_text(int(width/2), int(height/2), fill = SLS_FONT_COLOR, font = preview_font, anchor = CENTER, text = text, justify=CENTER, tags = ("stg_text",), width=width)
...
if True:
    ...
    sls_queue_treeview_frame.pack(side = LEFT, fill = Y, expand = 0)

    sls_tools_n_preview_frame = Frame(slideshow_mode_frame)
    sls_tools_n_preview_frame.pack(side = LEFT, fill = BOTH, expand = 1)
    ...
    sls_tool_btns_list = [[None, lambda: sls_pure_image("black"), "Černé [B]", "black"],
                          ...
                          [None, lambda: sls_pure_image("hide"), "Skrýt text [H]", "hide"],
    ...
    sls_queue_treeview.pack(fill=Y, expand=1)
    ...
    sls_temp_label = Label(slideshow_mode_frame)
    sls_temp_label.pack()
...
if True:
    ...
    stg_font_size_box["values"] = sizes_list
    stg_font_size_box["state"] = "readonly"
    stg_font_size_box.current(sizes_list.index(SLS_FONT_STYLE[1]))
    stg_font_size_box.pack(pady = 2)
    stg_font_size_box.bind("<<ComboboxSelected>>", func=stg_update_font_style)

    stg_font_style_box = ttk.Combobox(stg_options_frame)
    stg_font_style_box["values"] = text_style_list
    if len(SLS_FONT_STYLE) > 2:
        stg_font_style_box.current([style for style in STG_STYLE_DICT.values()].index(SLS_FONT_STYLE[2]))
    else:
        stg_font_style_box.current(2) # 2 je index "none"
    stg_font_style_box["state"] = "readonly"
    stg_font_style_box.pack(pady = 2)
    stg_font_style_box.bind("<<ComboboxSelected>>", func=stg_update_font_style)
...
sls_bind_list = [
    ...
    ["<h>", lambda event: sls_pure_image("hide"), None],
