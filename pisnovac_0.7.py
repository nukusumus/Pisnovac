...
popup_menu_item = None # uchova item v treeview pri kliknuti pravym tlacitkem
songlist_list : list = []
...
def sls_ordered_section_list(section_list: list, order: str) -> list[tuple]:
...
def sls_add_list_to_complete_list(path_list, clear_list = False):
    ...
    for song_path in path_list:
        ...
        sls_complete_list.append((song_file_name,sls_ordered_section_list(section_list=parsed_section_list, order=slide_order),))
...
def sls_switch_to_second_monitor():
    ...
    s = f"{selected_mon.width}x{selected_mon.height}+{selected_mon.x}+{selected_mon.y}"
    print(s)
...
if True:
    ...
    ### combobox pro vyber adresare ###
    sls_monitor_select_box = ttk.Combobox(sls_screen_select_frame)
    sls_monitor_select_box.pack(fill=X, expand = 0)
    sls_monitor_select_box["values"] = [f"Displej {get_monitors().index(mon) + 1} - {mon.width}x{mon.height}, {'primární' if mon.is_primary else 'vedlejší'}" for mon in get_monitors()]
    sls_monitor_select_box["state"] = "readonly"
    secondary_mon = get_monitors()[0]
    for mon in get_monitors():
        if not mon.is_primary:
            secondary_mon = mon
    sls_monitor_select_box.current(get_monitors().index(secondary_mon))
    ...
    # popup zobrazeni celeho textu sloky
    ...
    # popup pro zobrazeni obsahu seznamu
